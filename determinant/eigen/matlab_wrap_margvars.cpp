...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    if (nrhs != 5)
    {
        mexErrMsgTxt("Need 5 input arguments: matrix (upper triangular), precondition matrix, probingvector, cgTol, cgMaxIter");
    }
    if (nlhs>1)
    {
    ...
    i=1;
    MappedSparseMatrixType C(mxGetM(prhs[i]), mxGetM(prhs[i]), mxGetNzmax(prhs[i]), mxGetJc(prhs[i]), mxGetIr(prhs[i]), mxGetPr(prhs[i]));
    // SparseSelfAdjointView< MappedSparseMatrixType , Lower> M(C); // Preconditioner need not be SPD
    
    int *probePointer;
    probePointer = (int*)mxGetData(prhs[2]);
    Map<VectorXi> eigenProbe(probePointer,A.rows());
    
    
    double cgTol=mxGetScalar(prhs[3]);
    size_t cgMaxIter(static_cast<int>(mxGetScalar(prhs[4])));
    ...
    computeMargVars(A,C,eigenProbe,probeAinv,cgTol,cgMaxIter);
